...
public class AccountServiceTest {
    ...
    void testInsertAndDeleteSampleData() throws Exception {
        ...
        switch ("Account") {
            case "Customer": 
                stmt.executeUpdate("INSERT INTO customers (name, email, phone) VALUES ('Test User', 'test_account@example.com', '9999999999')");
                ResultSet rs = stmt.executeQuery("SELECT * FROM customers WHERE email='test_account@example.com'");
                assertTrue(rs.next(), "Inserted customers should exist in DB");
                stmt.executeUpdate("DELETE FROM customers WHERE email='test_account@example.com'");
                break;
            case "Account":
