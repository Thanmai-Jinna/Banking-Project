...
public class BranchServiceTest {
    ...
    void testInsertAndDeleteSampleData() throws Exception {
        ...
        switch ("Branch") {
            case "Customer": 
                stmt.executeUpdate("INSERT INTO customers (name, email, phone) VALUES ('Test User', 'test_branch@example.com', '9999999999')");
                ResultSet rs = stmt.executeQuery("SELECT * FROM customers WHERE email='test_branch@example.com'");
                assertTrue(rs.next(), "Inserted customers should exist in DB");
                stmt.executeUpdate("DELETE FROM customers WHERE email='test_branch@example.com'");
                break;
            case "Account":
                ...
                ps.executeUpdate();
                // cleanup
                stmt.executeUpdate("DELETE FROM account WHERE customer_id = " + cid);
                stmt.executeUpdate("DELETE FROM customer WHERE id = " + cid);
                break;
            case "Transaction":
                ...
                PreparedStatement psAcc = connection.prepareStatement("INSERT INTO accounts (customer_id, account_number, balance, type) VALUES (?, ?, ?, ?)", Statement.RETURN_GENERATED_KEYS);
