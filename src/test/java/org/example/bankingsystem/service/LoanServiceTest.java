...
public class LoanServiceTest {
    ...
    void testInsertAndDeleteSampleData() throws Exception {
        ...
        switch ("Loan") {
            case "Customer": 
                stmt.executeUpdate("INSERT INTO customers (name, email, phone) VALUES ('Test User', 'test_loan@example.com', '9999999999')");
                ResultSet rs = stmt.executeQuery("SELECT * FROM customers WHERE email='test_loan@example.com'");
                assertTrue(rs.next(), "Inserted customers should exist in DB");
                stmt.executeUpdate("DELETE FROM customers WHERE email='test_loan@example.com'");
                break;
            case "Account":
